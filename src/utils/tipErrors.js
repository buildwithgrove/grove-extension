...
(function() {
  ...
  class TipErrorHandler {
     ...
     * Delegates to ErrorBubble UI component if available
    ...
    static showInlineMessage(targetEl, message, variant = 'error') {
      // Use shared ErrorBubble component if available
      if (typeof ErrorBubble !== 'undefined') {
        ErrorBubble.show(targetEl, message, variant);
        return;
      }
      // Fallback: log error if ErrorBubble not loaded
      console.error('[TipErrorHandler] ErrorBubble not available:', message);
    }
