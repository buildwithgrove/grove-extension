...
const HistoryRenderer = {
  ...
  buildPlatformLink(tx, parsed, ctx) {
    ...
    // LinkedIn detection
    const isLinkedInFromContext = ctx.sender_platform === 'linkedin' ||
      (ctx.source_post_url && this.isLinkedInUrl(ctx.source_post_url));
    const isLinkedInFromDestination = this.isLinkedInUrl(parsed.profileUrl);
    const isLinkedInFromSocialGraph = this.isLinkedInUrl(tx.social_graph);
    const isLinkedIn = isLinkedInFromContext || isLinkedInFromDestination || isLinkedInFromSocialGraph;

    if (!platformIcon && isLinkedIn) {
      platformTitle = 'View on LinkedIn';
      platformIcon = this.icons.linkedin;
      if (ctx.source_post_url) {
        platformUrl = ctx.source_post_url;
      } else if (isLinkedInFromDestination) {
        platformUrl = parsed.postUrl || parsed.profileUrl;
      } else if (isLinkedInFromSocialGraph) {
        platformUrl = tx.social_graph.startsWith('http') ? tx.social_graph : `https://${tx.social_graph}`;
      }
    }
