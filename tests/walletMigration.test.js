...
function getMigrateFunction() {
  ...
  return async function migrateWalletStorageKeys() {
    ...
    await chrome.storage.local.set({
      [STORAGE_KEYS.WALLET_KEYS_MIGRATED]: true,
    });
    if (Object.keys(updates).length) {
      console.log(
...
describe("migrateWalletStorageKeys", () => {
  ...
  it("should skip migration if already migrated", async () => {
    // Pre-set the migrated flag
    await mockChrome.storage.local.set({
      GROVE_WALLET_KEYS_MIGRATED: true,
      GROVE_CLIENT_ADDRESS: "0xShouldNotMigrate",
    });

    const migrate = getMigrateFunction();
    await migrate();

    // Old key should still exist (migration was skipped)
    const old = await mockChrome.storage.local.get(["GROVE_CLIENT_ADDRESS"]);
    expect(old["GROVE_CLIENT_ADDRESS"]).toBe("0xShouldNotMigrate");

    // New key should not have been set
    const result = await mockChrome.storage.local.get([
      "GROVE_EARNING_ADDRESS",
    ]);
    expect(result["GROVE_EARNING_ADDRESS"]).toBeUndefined();
  ...
  it("should set migrated flag after successful migration", async () => {
    await mockChrome.storage.local.set({
      GROVE_CLIENT_ADDRESS: "0xClient",
    });
    ...
    const flags = await mockChrome.storage.local.get([
      "GROVE_WALLET_KEYS_MIGRATED",
    ]);
    expect(flags["GROVE_WALLET_KEYS_MIGRATED"]).toBe(true);
  });
